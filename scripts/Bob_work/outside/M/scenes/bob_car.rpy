...
screen bob_car_scr:
    ...
    imagebutton:
        ...
        if bob_carkeys.selected:
            hovered Show("displayTextScreen", displayText = "Открыть автомобиль")
            if clickable == True:
                activate_sound "sfx/car_open.wav"
                action [Hide("displayTextScreen"),Jump("bob_car_label2")]
                unhovered Hide("displayTextScreen")
        else:
            hovered Show("displayTextScreen", displayText = "Дверь автомобиля")
            if clickable == True:
...
label bob_carbook:
    ...
    $ clickable = False
    MC "Что здесь делает эта книга? Похоже, это из офиса папы. Может быть, папе это понадобится. Я должен вернуть ее в офис."
    $ bob_carbook_s = 2
