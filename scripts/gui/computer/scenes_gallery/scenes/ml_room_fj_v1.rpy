...
label ml_bedroom_morning_scene6_v1_label_after_ch1r:
    ...
    Mom "(Он явно наслаждается, когда я щелкаю языком по нему.)"

    scene ml_bedroom_morning_scene6_v1_p16
    if renpy.loadable("patch.rpy"):
        MC "(Я сейчас такой возбудженный ... не могу поверить, что мама так долго меня привлекала.)"
    if not renpy.loadable("patch.rpy"):
        MC "(Я сейчас такой возбудженный ... не могу поверить, что Линда так долго меня привлекала.)"
...
label ml_bedroom_morning_scene6_v1_label_after_ch21r:
    ...
    if renpy.loadable("patch.rpy"):
        MC "Можешь раздвинуть ноги еще немного, мама?"
    if not renpy.loadable("patch.rpy"):
