...
screen inventory_screen:
    ...
    imagebutton:
        ...
        action [Hide("inventory_screen"),Hide("display_Item_Name"), Hide("displayTextScreen"), Play ("sound", "sfx/zipper.mp3")]
    imagebutton:
        xalign 0.5
        yalign 0.5
        focus_mask True
    ...
    vbox xpos 1120 ypos 292 spacing 20:
        frame:
            style "frame_gui1"
    ...
    for item in sorted_items:
        if i+1 <= (inv_page+1)*12 and i+1>inv_page*12:
            ...
            $ pich = item.hover_i
            $ my_tooltip = "tooltip_inventory_" + pic.replace("images/inv_", "").replace(".png", "")


            if item.note==True:
                imagebutton idle pic hover pich xpos x ypos y action [Hide("display_Item_Name"),  SetVariable("item", item), Show("note_view")] hovered [ Play ("sound", "sfx/click.wav"), Show("display_Item_Name", my_picture=my_tooltip, my_tt_ypos=y, my_tt_xpos=x, displayText1 = [item.name]),] unhovered [Hide("display_Item_Name")] at inv_eff
            if item.note==False:
                imagebutton idle pic hover pich xpos x ypos y action [Show("display_Item_Name", my_picture=my_tooltip, my_tt_ypos=y, my_tt_xpos=x, displayText1 = [item.name]),  SetVariable("item", item), selectItem(item)] hovered [ Play ("sound", "sfx/click.wav"), Show("display_Item_Name", my_picture=my_tooltip, my_tt_ypos=y, my_tt_xpos=x, displayText1 = [item.name]),] unhovered [Hide("display_Item_Name")] at inv_eff
            if item.selected:
